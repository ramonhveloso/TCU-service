...
async def test_get_extra_expenses(use_test_client):
    ...
    json_request = {"amount": 100.00, "description": "teste", "date": datetime.now()}

    post_expenses_response = use_test_client.post(
        "/api/v1/extra-expenses/", headers=headers, params=json_request
    )
    assert post_expenses_response.status_code == 201

    get_expenses_response = use_test_client.get(
        "/api/v1/extra-expenses/", headers=headers
    )
...
async def test_get_by_id_extra_expenses(use_test_client):
    ...
    get_expenses_response = use_test_client.get(
        f"/api/v1/extra-expenses/{extra_expense_id}", headers=headers
    )
    assert get_expenses_response.status_code == 200
...
async def test_put_by_id_extra_expenses(use_test_client):
    ...
    put_expenses_response = use_test_client.put(
        f"/api/v1/extra-expenses/{extra_expense_id}", headers=headers, json=json_request
    )
...
async def test_post_multiple_extra_expenses(use_test_client):
    ...
    # Inserindo extra expenses
    json_request = [
        {"amount": 100.00, "description": "teste", "date": str(datetime.now())},
        {"amount": 120.00, "description": "teste 1", "date": str(datetime.now())},
        {"amount": 130.00, "description": "teste 2", "date": str(datetime.now())},
    ]

    post_expenses_response = use_test_client.post(
        "/api/v1/extra-expenses/multiple", headers=headers, json=json_request
    )
    assert post_expenses_response.status_code == 201
