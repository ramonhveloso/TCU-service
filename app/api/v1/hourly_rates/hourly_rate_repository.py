...
from app.api.v1.hourly_rates.hourly_rate_schemas import PostHourlyRateRequest, PutHourlyRateRequest
...
class HourlyRateRepository:
    async def get_all_hourly_rates(self, user_id: int, db: Session):
        return db.query(HourlyRate).filter(HourlyRate.user_id == user_id, HourlyRate.deleted_at == None).all()
    
    async def get_hourly_rate_by_id(self, db: Session, user_id: int, hourly_rate_id: int):
        return db.query(HourlyRate).filter(HourlyRate.id == hourly_rate_id, HourlyRate.user_id == user_id, HourlyRate.deleted_at == None).first()
    ...
    async def update_hourly_rate(self, db: Session, existing_hourly_rate: HourlyRate, hourly_rate: PutHourlyRateRequest):
        existing_hourly_rate.rate = hourly_rate.rate if hourly_rate.rate else existing_hourly_rate.rate
