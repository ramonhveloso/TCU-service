...
class ExtraExpenseService:
    ...
    async def post_extra_expenses(self, db: Session, user_id: int, extra_expenses: PostExtraExpensesRequest) -> PostExtraExpensesResponse:
        list_extra_expenses = []
        list_extra_expenses_errors = []

        for extra_expense in extra_expenses.extra_expenses:
            try:
                response_repository = await self.extra_expense_repository.post_extra_expense(
                    db=db, user_id=user_id, extra_expense=extra_expense
                )
                list_extra_expenses.append(self._build_response_data(response_repository))
    ...
    async def update_extra_expense(
        ...
        existing_extra_expense = await self.extra_expense_repository.get_extra_expense_by_id(db=db, user_id=user_id, extra_expense_id=extra_expense_id)
        if not extra_expense:
            raise HTTPException(status_code=404, detail="Extra expense not found")
    ...
    async def delete_extra_expense(self, db: Session, user_id: int, extra_expense_id: int) -> DeleteExtraExpenseResponse:
        extra_expense = await self.extra_expense_repository.get_extra_expense_by_id(db=db, user_id=user_id, extra_expense_id=extra_expense_id)
        if not extra_expense:
            raise HTTPException(status_code=404, detail="Extra expense not found")
