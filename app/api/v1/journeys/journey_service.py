...
class JourneyService:
    ...
    async def get_all_journeys(self, db: Session, user_id: int) -> GetJourneysResponse:
        journeys = await self.journey_repository.get_all_journeys(user_id=user_id, db=db)
        if not journeys:
    ...
    async def get_journey_by_id(self, db: Session, user_id: int, journey_id: int) -> GetJourneyResponse:
        journey = await self.journey_repository.get_journey_by_id(db=db, user_id=user_id, journey_id=journey_id)
        if not journey:
            raise HTTPException(status_code=404, detail="Journey not found")
        return GetJourneyResponse(id=journey.id,
                                    user_id=journey.user_id,
                                    start=journey.start,
                                    end=journey.end,
                                    hours_worked=journey.hours_worked,
    ...
    async def post_journey(self, db: Session, user_id: int, journey: PostJourneyRequest) -> PostJourneyResponse:
        try:
            response_repository = await self.journey_repository.post_journey(db=db, user_id=user_id, journey=journey)
        except Exception:
            raise HTTPException(status_code=400, detail="Journey not created")
        return PostJourneyResponse(message="Journey created successfully",
    ...
    async def post_journeys(self, db: Session, user_id: int, journeys: PostJourneysRequest) -> PostJourneysResponse:
        list_journeys = []
        list_journeys_errors = []
        ...
        return PostJourneysResponse(
            message="Journeys created with some errors" if list_journeys_errors else "Journeys created successfully",
    ...
    async def update_journey(
        ...
        existing_journey = await self.journey_repository.get_journey_by_id(db=db, user_id=user_id, journey_id=journey_id)
        if not journey:
            raise HTTPException(status_code=404, detail="Journey not found")

        updated_journey = await self.journey_repository.update_journey(db=db, existing_journey=existing_journey, journey=journey)
    ...
    async def delete_journey(self, db: Session, user_id: int, journey_id: int) -> DeleteJourneyResponse:
        journey = await self.journey_repository.get_journey_by_id(db=db, user_id=user_id, journey_id=journey_id)
        if not journey:
            raise HTTPException(status_code=404, detail="Journey not found")
